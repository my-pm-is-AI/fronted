...
function CylinderText() {
  ...
  const phrase = "ORCHESTRATE YOUR AI WORKFORCE · ";
  const repeatedPhrase = phrase.repeat(3);
  const chars = repeatedPhrase.split("");
  const radius = 3;
    ...
    return chars.map((char, i) => {
      ...
      return (
        <Text
          ...
          anchorX="center"
          anchorY="middle"
          font="/fonts/GeistMono-Regular.otf"
        >
          {char}
        </Text>
      );
  ...
  return <group ref={groupRef}>{charElements}</group>;
}

function GlitchText3D() {
  const groupRef = useRef<THREE.Group>(null);
  const words = ["ORCHESTRATE", "YOUR", "AI", "WORKFORCE"];
