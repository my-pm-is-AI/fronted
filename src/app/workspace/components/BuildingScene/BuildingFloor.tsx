...
export default function BuildingFloor({ floorIndex, children }: BuildingFloorProps) {
  ...
  return (
    <group position={[0, floorIndex * (ROOM_HEIGHT + FLOOR_THICKNESS), 0]}>
      ...
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.01, 0]} receiveShadow>
        ...
        <meshStandardMaterial color='#c8d0e8' roughness={0.25} metalness={0.05} />
      </mesh>
      {/* Back wall */}
      <mesh position={[0, ROOM_HEIGHT / 2, -half + WALL_THICKNESS / 2]} castShadow receiveShadow>
        ...
        <meshStandardMaterial color={COLORS.wallBack} roughness={0.75} />
      </mesh>
      <mesh position={[0, ROOM_HEIGHT / 2, -half + WALL_THICKNESS + 0.01]}>
        <planeGeometry args={[ROOM_SIZE, ROOM_HEIGHT]} />
        <meshStandardMaterial color={COLORS.wallBackLight} roughness={1} />
      </mesh>
      {/* Left wall */}
      <mesh position={[-half + WALL_THICKNESS / 2, ROOM_HEIGHT / 2, 0]} castShadow receiveShadow>
        ...
        <meshStandardMaterial color={COLORS.wallSide} roughness={0.75} />
      </mesh>
      ...
      <mesh position={[0, ROOM_HEIGHT + FLOOR_THICKNESS / 2, 0]}>
        ...
        <meshStandardMaterial color={COLORS.ceilingBase} roughness={0.9} />
      </mesh>
      ...
      <mesh position={[0, ROOM_HEIGHT + trimH, half]}>
        ...
        <meshStandardMaterial color={COLORS.trimWhite} emissive={COLORS.trimGlow} emissiveIntensity={0.7} />
      </mesh>
      <mesh position={[0, ROOM_HEIGHT + trimH, -half]}>
        <boxGeometry args={[ROOM_SIZE + 0.08, 0.07, 0.08]} />
        <meshStandardMaterial color={COLORS.trimWhite} emissive={COLORS.trimGlow} emissiveIntensity={0.4} />
      ...
      <mesh position={[0, 0.05, half - 0.1]}>
        ...
        <meshStandardMaterial color={COLORS.neonBlue} emissive={COLORS.neonBlue} emissiveIntensity={2} />
      </mesh>
