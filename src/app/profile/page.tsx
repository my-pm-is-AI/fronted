...
description: &gt;
  连接本地 OpenClaw 实例到 AgentOS 平台，实现任务监听、进度同步和真人介入请求。
...
# AgentOS OpenClaw 连接器

本技能用于将本地 OpenClaw 实例接入 AgentOS 工作流平台，实现自动化任务处理与人工协作的无缝衔接。

## 核心能力

1. **任务监听与接单** - 轮询 AgentOS 任务队列，自动认领分配给当前 OpenClaw 实例的任务
...
- 已获取 AgentOS API 访问令牌（"AGENTOS_API_TOKEN"）
- 本地 OpenClaw 实例已启动并可访问（默认 "http://localhost:11434"）
- 网络可连通 "api.agentos.dev"

## 工作流程

### 1. 任务监听循环 (Task Polling)

当用户要求"开始监听任务"或"接入 AgentOS"时，启动以下流程：
